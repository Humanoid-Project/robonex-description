...
from model_io import load_urdf, load_loops, fmt, ROOT
...
def emit_joint(o, j):
    ...
    o.append("        </limit>")
    o.append("        <dynamics><damping>%g</damping><friction>%g</friction></dynamics>"
...
def main():
    parser = argparse.ArgumentParser(
        description="Build the RoboNex Gazebo model and world SDF files."
    )
    parser.add_argument(
        "--drop",
        action="store_true",
        help="use the MuJoCo spawn height (1.085 m) instead of the ground pose",
    )
    args = parser.parse_args()
    height = MUJOCO_SPAWN_HEIGHT if args.drop else SPAWN_HEIGHT
    ...
    print("  base           : free-floating at z = %.4f m%s"
          % (height, "  (--drop: MuJoCo height)" if args.drop else "  (soles on the ground)"))
    print()
